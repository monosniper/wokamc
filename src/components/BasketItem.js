...
const BasketItem = ({item}) => {
    const {
        main: { tags, products },
        basket: { getItemPrice, addCount, remove },
    } = useStores();

    const product = products.find(({id, title}) => item.id === 'money' ? title === 'money' : id === item.id)
    ...
    return (
        <div className="cart-product">
            ...
            <div className="cart-product__info">
                ...
                <div className="cart-product__clm">
                    <div className="cart-product__price">{item.id === 'money' ? item.amount : getItemPrice(item, product)}.00 ₽</div>
                    <div className="quantity" data-quantity="">
                        ...
                        <div className="quantity__input"><input
                            ...
                            name="form[]" defaultValue={item.count} /></div>
                        {tags.find(tag => tag.id === product.TagId).isPrivilege || item.id === 'money' || product.Tag.isAlone ? null : (
                            <button data-quantity-plus={true} type="button" onClick={() => addCount(item.id)}
